...
public class ApiResource {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Category categoryByName(@PathParam("category-name") String categoryName, @Context HttpServletRequest httpRequest) {
        try {
            Category category = categoryDao.findByName(categoryName);
            if(category == null) {
                throw new ApiException("Category name does not exist!");
            }
            return category;
        }
        catch (Exception e) {
            throw new ApiException("Search by category name failed...");
        }
    }
    ...
    public List<Book> booksByCategoryName(
        ...
        try {
            Category category = categoryDao.findByName(categoryName);
            if (category == null) {
                throw new ApiException(String.format("No such category name: %s exists", categoryName));
            }
            return bookDao.findByCategoryId(category.getCategoryId());

        } catch (Exception e) {
            throw new ApiException(String.format("Books lookup based via categoryName %s failed", categoryName), e);
        }
